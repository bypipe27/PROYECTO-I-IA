...
# ---------------------------------------------------------------------------
# Panel del subgrafo A* (panel derecho superior)
# ---------------------------------------------------------------------------

class PanelSubgrafo(QWidget):
    """
    Muestra el subgrafo del acertijo (subproblema A*) con costos en las
    aristas, resaltando el nodo actual conforme avanza el algoritmo.
    Se actualiza cada vez que la búsqueda global encuentra un nodo bloqueado.
    """
    ...
    def __init__(self, parent=None):
        ...
        self._titulo_lbl = QLabel("Puzzle Solver — A*")
    ...
    def cargar_subgrafo(self, resultado_local: dict, nodo_bloqueado: str):
        """
        Recibe el resultado_local del A* (que incluye 'grafo_local' y
        'heuristica') y dibuja el subgrafo completo con costos en aristas.
        """
        ...
        self._titulo_lbl.setText(
            f"Puzzle Solver — A*   (desbloqueando: {nodo_bloqueado})"
        )
        self._redibujar()
    ...
    def _redibujar(self):
        ...
        # Calcular posiciones lineales para el subgrafo (es pequeño y lineal)
        ...
        for nodo, (x, y) in posiciones.items():
            ...
            # Etiqueta corta: solo la parte después de "_"
            etiqueta = nodo.split("_")[-1] if "_" in nodo else nodo
            item = NodoItem(etiqueta, x, y, color)
            self._scene.addItem(item)
            self._items[nodo] = item
            ...
            if h_val != "":
                h_lbl = QGraphicsTextItem(f"h={h_val}")
    ...
    def _calcular_pos_subgrafo(self) -> dict:
        """
        Distribución horizontal simple para el pequeño subgrafo A*.
        El grafo tiene forma: S -> P1/P2 -> G
        """
        from collections import deque
    ...
    def limpiar(self):
        ...
        self._inicio_local = ""
        self._obj_local    = ""
        self._titulo_lbl.setText("Puzzle Solver — A*")
        self._mostrar_placeholder()
